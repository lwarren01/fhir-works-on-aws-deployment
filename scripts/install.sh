...
if [[ "${IMPORT_PRIVATE_API_GATEWAY}" == "true" ]]; then
    ...
    if [ "$has_resources" = false ]; then
        ...
        cat /tmp/fhir_service_template.json | jq -r '.Resources +={"FHIRServicePrivateStage":{"Type":"AWS::ApiGateway::Stage","Condition":"isUsingPrivateApi","DependsOn":["FHIRServicePrivate","FhirServerLambdaFunction","FhirServerProvConcLambdaAlias","FHIRServiceStageDeployment"],"Properties":{"RestApiId":{"Ref":"FHIRServicePrivate"},"StageName":{"Ref":"Stage"},"DeploymentId":{"Ref":"FHIRServiceStageDeployment"},"AccessLogSetting":{"DestinationArn":{"Fn::GetAtt":["ApiGatewayLogGroup","Arn"]},"Format":"${self:provider.logs.restApi.format}"},"TracingEnabled":"${self:provider.tracing.apiGateway}","MethodSettings":[{"ResourcePath":"/*","HttpMethod":"*","MetricsEnabled":true,"LoggingLevel":"OFF","DataTraceEnabled":true}]}}}' > /tmp/fhir_service_template.json.tmp && mv /tmp/fhir_service_template.json.tmp /tmp/fhir_service_template.json
